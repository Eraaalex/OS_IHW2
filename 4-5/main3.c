...
//}
//
//void setup_shared_resources(int initial_value, int **shared_memory) {
//    sem_unlink(SEM_WRITER);
//    sem_unlink(SEM_READER);
//
//    sem_t *writer = sem_open(SEM_WRITER, O_CREAT, 0666, 1);
//    sem_t *reader = sem_open(SEM_READER, O_CREAT, 0666, initial_value);
//    if (writer == SEM_FAILED || reader == SEM_FAILED) {
...
//    }
//
//    ftruncate(shm_fd, sizeof(int) * initial_value);
//    *shared_memory = (int *) mmap(NULL, sizeof(int) * initial_value, PROT_READ | PROT_WRITE, MAP_SHARED, shm_fd, 0);
...
//}
//
//void writer_process( int max_value, int *shared_memory) {
//    sem_t *writer = sem_open(SEM_WRITER, 0);
//    sem_t *reader = sem_open(SEM_READER, 0);
//
...
//        array[pos] = new_val;
//        printf("Writer [PID = %d] changed index %d from %d to %d\n", getpid(), pos, old_val, new_val);
//        sem_post(writer);
//        sleep(2);
//    }
//    exit(0);
//}
//
//void reader_process(int max_value, int *shared_memory) {
//    sem_t *reader = sem_open(SEM_READER, 0);
//
//    int *array = (int *) shared_memory;
//    srand(time(NULL) ^ (getpid()<<16));
...
//        sem_wait(reader);
//        int pos = rand() % max_value;
//        printf("Reader [PID = %d] read index %d with value %d, Fibnacci[%lld] = %lld \n", getpid(), pos, array[pos], fib(array[pos]));
//        sem_post(reader);
//        sleep(2);
//    }
//    exit(0);
...
//
//
//    setup_shared_resources(10, &shared_memory);
//
//    for (int i = 0; i < num_readers; ++i) {
//        if (fork() == 0) {
//            reader_process(SIZE, shared_memory);
//        }
//    }
//
//    for (int i = 0; i < num_writers; ++i) {
//        if (fork() == 0) {
//            writer_process(SIZE, shared_memory);
//        }
//    }
//
